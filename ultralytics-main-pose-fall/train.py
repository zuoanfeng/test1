...
"""
1、模型路径，即yolov8.yaml， 要是修改成其它模型，在ultralytics/cfg/models/v8 找
2、修改数据集文件路径，即  data=r'VisDrone.yaml'  数据集路径参考ultralytics/cfg/datasets/VisDrone.yaml
3、修改batch、workers，电脑性能不行，就调低
4、project='runs/person' 这个是训练结果存放路径
"""
...
if __name__ == '__main__':
    ...
    model.train(
        data=r'ultralytics/cfg/datasets/VisDrone.yaml',  # 文件放在ultralytics/cfg/datasets
        epochs=150,  # (int) 训练的周期数
        patience=30,  # (int) 等待无明显改善以进行早期停止的周期数
        ...
        device='cpu',  # (int | str | list, optional) 运行的设备，例如 cuda device=0 或 device=0,1,2,3 或 device=cpu
        workers=4,  # (int) 数据加载的工作线程数（每个DDP进程）
        project='runs/test',  # (str, optional) 项目名称
        ...
        # 超参数 ----------------------------------------------------------------------------------------------
        lr0=0.001,  # (float) 初始学习率（例如，SGD=1E-2，Adam=1E-3）
